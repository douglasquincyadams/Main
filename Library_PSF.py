...
def SplineFitPSF(PSFelback=None,
                 PSFelfront=None,
                 ConfidenceLevel=None,
                 ):

    r200=np.linspace(0.,4,42)
    combinedBackAndFront=np.zeros(len(r200))
    for i,r200i in enumerate(r200):
        back1=CombinedPSF(r200=r200i,PSFel=PSFelback,ScalingParam='Back',PhotonFrac=ConfidenceLevel, )
        front1=CombinedPSF(r200=r200i,PSFel=PSFelfront,ScalingParam='Front',PhotonFrac=ConfidenceLevel, )
        combinedBackAndFront[i] = (back1+front1)/2.
        #print 'combinedBackAndFront', combinedBackAndFront[i]
    PSFspline=UnivariateSpline(r200,combinedBackAndFront,s=0)
    #plt.plot(r200,r200,c='red')               
    #plt.plot(r200,combinedBackAndFront)
    #r200Extended=np.linspace(0,11,102)
    #plt.plot(r200Extended,PSFspline(r200Extended),c='blue')
    #plt.show()
    return PSFspline
        

def Main(DMMass=None,ConfidenceLevel=None,):
    ...
    if DMMass==None:
        ...
        PSFelback=AvePSFelments(DMMass=DMMass,SpDic=SpDicback, filenameLTC=filenameLTC,)
        PSFelfront=AvePSFelments(DMMass=DMMass,SpDic=SpDicfront, filenameLTC=filenameLTC,)
        #otherwise Return Spline 
        PSFspline=SplineFitPSF(PSFelback=PSFelback,
                    PSFelfront=PSFelfront,
                    ConfidenceLevel=ConfidenceLevel,
                    )
        return PSFspline #note that expansion is interms of Degrees
